...
v_5in10_fix_shared <- res_HNA_5in10_fix %>%
...
v_HNA_5in10_fix_uniq <- res_HNA_5in10_fix %>%
  filter(!taxa %in% unique(res_LNA_5in10_fix$taxa)) %>% 
  ggplot2::ggplot(ggplot2::aes(`FL1-H`, `FL3-H`, z = cor))+
  ggplot2::geom_tile(ggplot2::aes(fill=cor)) +
  # ggplot2::geom_point(size = 1, ggplot2::aes(fill=cor), shape =22,
...
v_LNA_5in10_fix_uniq <- res_LNA_5in10_fix %>%
...
png(file = "FPcorSEQ_5in10_partition.png", width = 25, height = 10, units  = "in", res = 500)
grid.arrange(v_LNA_5in10_fix_uniq, v_5in10_fix_shared, v_HNA_5in10_fix_uniq,
             ncol = 3)
dev.off()
```
