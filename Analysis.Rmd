...
# Figure 1
```{r boxplot, fig.width = 5.5, fig.height = 2.75}
...
```{r prod-HNALNA, fig.width = 6.5, fig.height = 2.75,}
...
```{r fig1, fig.width = 6.5, fig.height = 5.2}
...
mich_top10_OTUs <- michigan_dat_scores %>%
  top_n(., 10, Michigan_HNA) %>%
  dplyr::select(OTU)

inland_top10_HNA_OTUs <- inland_dat_scores %>%
  dplyr::select(OTU, Inland_HNA) %>%
  top_n(., 10, Inland_HNA) %>%
  dplyr::select(OTU)

inland_top10_LNA_OTUs <- inland_dat_scores %>%
  dplyr::select(OTU, Inland_LNA) %>%
  top_n(., 10, Inland_LNA) %>%
  dplyr::select(OTU) 
  
musk_top10_HNA_OTUs <- musk_dat_scores %>%
  dplyr::select(OTU, Muskegon_HNA) %>%
  top_n(., 10, Muskegon_HNA) %>%
  dplyr::select(OTU)

musk_top10_LNA_OTUs <- musk_dat_scores %>%
...
```{r fig4columnsOnly, fig.width=6, fig.height=8}
...
### ggplot heatmap 
```{r fig4-ggheat-top10, fig.width=5, fig.height=7}
# In the order of OTUs without 
lake_ordered_OTUs <- row.names(matrix_top10)

df_top10 %>% 
  mutate(OTU = str_replace(OTU, "00", ""),
         OTU = as.factor(OTU),
         OTU = fct_relevel(OTU, lake_ordered_OTUs)) %>% 
...
######################################### FASTTREE
fast_tree <- read.tree(file="data/fasttree/Figure5/newick_tree_OTUs155.tre")
...
#write.csv(df_RLscores, file = "data/fasttree/Figure5/OTUs155_MANUAL.csv", row.names = FALSE)
# Next I manually filled in the fcm_type and lake columns :/ 

phyfcm_fasttree_df <- read.csv("data/fasttree/Figure5/OTUs155_MANUAL.csv") %>%
...
gheatmap(rooted_tree, phyfcm_fasttree_df, offset = 0.1, width=0.3, font.size=3, colnames_angle=0, hjust=0.5) +
...
## Figure 5 with Top 10 OTUs from Each System
```{r fig5-top10, fig.width=7, fig.height=9}
# Get the OTU names
top10_otu_scores_df <- df_top10 %>%
  as.data.frame() 
top10_vector_of_otus <- as.vector(top10_otu_scores_df$OTU)

# Write the OTU names to a file
#write(top10_vector_of_otus, file = "data/fasttree/Figure5/OTUnames_RLscores_top10.txt",
#      ncolumns = 1, append = FALSE, sep = "\n")


top10_physeq <- physeq.otu %>%
  subset_taxa(., taxa_names(physeq.otu) %in% top10_vector_of_otus)  

# Which OTU is missing?
setdiff(sort(top10_vector_of_otus), sort(taxa_names(top10_physeq)))
setdiff(sort(taxa_names(top10_physeq)), sort(top10_vector_of_otus))

######################################### FASTTREE
top10_fasttree <- read.tree(file="data/fasttree/Figure5/newick_tree_top10.tre")

top10_fasttree_tip_order <- data.frame(top10_fasttree$tip.label) %>%
  rename(OTU = top10_fasttree.tip.label)

top10_fasttree_physeq <- merge_phyloseq(physeq, phy_tree(top10_fasttree))

# Fix the taxonomy names 
colnames(tax_table(top10_fasttree_physeq)) <- c("Kingdom","Phylum","Class","Order","Family","Genus","Species")

###################################################################### ADD THE PROTEOBACTERIA TO THE PHYLA
phy <- data.frame(tax_table(top10_fasttree_physeq))
Phylum <- as.character(phy$Phylum)
Class <- as.character(phy$Class)
...
t <- tax_table(as.matrix(phy))
tax_table(top10_fasttree_physeq) <- t

top10_fasttree_tax <- data.frame(tax_table(top10_fasttree_physeq)) %>%
  tibble::rownames_to_column(var = "OTU")

# Make a dataframe 
phymerge_df <- phyfcm_fasttree_df %>%
  tibble::rownames_to_column("OTU")

phyfcm_top10_df <- 
  top10_otu_scores_df %>%
  select(OTU) %>%
  left_join(phymerge_df, by = "OTU") %>%
  tibble::column_to_rownames("OTU")
  

## Let's root the tree
is.rooted(top10_fasttree)
top10_rooted_tree <- root(top10_fasttree, outgroup = "Otu001533", resolve.root = TRUE)
is.rooted(top10_rooted_tree) # Sanity Check

top10_rooted_tree_plot <- 
  ggplot(top10_rooted_tree, aes(x, y)) + geom_tree() + theme_tree() +
  geom_tiplab(size=3, align=TRUE, linesize=.5) #+
  #geom_nodelab(vjust=-.5, size=3) 

gheatmap(top10_rooted_tree_plot, phyfcm_top10_df, offset = 0.1, width=0.3, font.size=3, colnames_angle=0, hjust=0.5) +
  scale_fill_manual(values = phylum_colors, 
                    breaks = c("Actinobacteria", "Alphaproteobacteria", "Bacteria_unclassified", "Bacteroidetes", 
                               "Betaproteobacteria", "Chlorobi","Chloroflexi", "Cyanobacteria", "Deltaproteobacteria", 
                               "Epsilonproteobacteria", "Firmicutes", "Gammaproteobacteria", "Lentisphaerae", "Omnitrophica", 
                               "Planctomycetes", "Proteobacteria_unclassified", "unknown_unclassified", "Verrucomicrobia", 
                               # FCM Groups
                               "Both", "HNA", "LNA")) +
  guides(fill=guide_legend(ncol=1))
```
