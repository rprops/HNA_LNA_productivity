...
```{r discrete-phylosig}
# Discrete models: https://www.r-phylo.org/wiki/HowTo/InferringModelsForDiscreteData
discrete_FCM <- FCM_categorical_df$FCM
names(discrete_FCM) <- FCM_categorical_df$OTU

# Equal transition rates model with a Juke's Cantor Model 
discrete_ER_fit <- fitDiscrete(rooted_tree, discrete_FCM, model = "ER")
# Asymmetrical transition rates (ARD = "all rates different")
discrete_ARD_fit <- fitDiscrete(rooted_tree, discrete_FCM, model = "ARD")
1-pchisq(2*(discrete_ARD_fit$opt$lnL-discrete_ER_fit$opt$lnL),1) # ARD fit is better than ER
# Discrete Pagel's Lambda
discrete_lambda_fit <- fitDiscrete(rooted_tree, discrete_FCM, treeTransform = "lambda")
1-pchisq(2*(discrete_lambda_fit$opt$lnL-discrete_ARD_fit$opt$lnL),1) # ARD and Pagel's lambda don't differ in the log likelihoods

optimal <- fitDiscrete(rooted_tree, discrete_FCM, treeTransform = "lambda")
lambda0_phy <- rescale(rooted_tree, "lambda", 0) # A big polytomy tree 
zero <- fitDiscrete(lambda0_phy, discrete_FCM)
...
HNAscores <- summarized_RL_scores$HNA_score
names(HNAscores) <- summarized_RL_scores$OTU
phylosig(rooted_tree, HNAscores, method = "lambda", test = TRUE, nsim = 999)

# Does the LNA RL Score have phylogenetic signal?
LNAscores <- summarized_RL_scores$LNA_score
names(LNAscores) <- summarized_RL_scores$OTU
phylosig(rooted_tree, LNAscores, method = "lambda", test = TRUE, nsim = 999)

# Calculating Pagel's lambda in geiger
BM_HNA <- fitContinuous(rooted_tree, HNAscores, model = "BM")
lambda_HNA <- fitContinuous(rooted_tree, HNAscores, model = "lambda")
pchisq(-2*(lambda_HNA$opt$lnL - BM_HNA$opt$lnL),1,lower.tail=F)

plot(lambdaTree(rooted_tree, lambda = 2))

BM_LNA <- fitContinuous(rooted_tree, LNAscores, model = "BM")
lambda_LNA <- fitContinuous(rooted_tree, LNAscores, model = "lambda")
pchisq(-2*(lambda_LNA$opt$lnL - BM_LNA$opt$lnL),1,lower.tail=F)
```
...
# HNA
# Subset all of the HNA OTUs
HNA_df_phydist <- summarized_RL_scores %>%
  select(OTU, HNA_score) %>%
  na.omit()

# Trip the larger tree to only have the HNA OTUs
HNA_rooted_tree <- drop.tip(phy = rooted_tree, tip = rooted_tree$tip.label[-match(HNA_df_phydist$OTU, rooted_tree$tip.label)])
...
# Calculate the pairwise distances of the OTUs in the HNA phylogeny
HNA_rooted_phy_dist <- cophenetic(HNA_rooted_tree)

# Do HNA OTUs have phylogenetic autocorrelation? 
# Moran's I test 
...
# Trip the larger tree to only have the HNA OTUs
LNA_rooted_tree <- drop.tip(phy = rooted_tree, tip = rooted_tree$tip.label[-match(LNA_df_phydist$OTU, rooted_tree$tip.label)])
...
# Do LNA OTUs have phylogenetic autocorrelation? 
# Moran's I
...
# HNA
# Perform an Abouheif moran test using Monte Carlo simulations (default is 999 randomizations)
abouheif_HNA_test <- abouheif.moran(x = HNA_df_phydist$HNA_score, W = HNA_rooted_phy_dist, method = "oriAbouheif", nrepet = 999)
abouheif_HNA_test
plot(abouheif_HNA_test)

# LNA
# Perform an Abouheifmoran test using Monte Carlo simulations (default is 999 randomizations)
abouheif_LNA_test <- abouheif.moran(x = LNA_df_phydist$LNA_score, W = LNA_rooted_phy_dist, method = "oriAbouheif", nrepet = 999)
abouheif_LNA_test
plot(abouheif_LNA_test)
```
